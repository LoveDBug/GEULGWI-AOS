...
class SignUpViewModelTest {
    ...
    @Test
    fun `비밀번호 및 확인 불일치 처리`() = runTest {
        val vm = SignUpViewModel(FakeNavigator())

        vm.container.stateFlow.test {
            awaitItem()
            // move to Password step
            vm.onEmailChanged("a@a.com"); awaitItem()
            vm.onNextStep(); awaitItem()
            vm.onCodeChanged("123456"); awaitItem()
            vm.onNextStep()
