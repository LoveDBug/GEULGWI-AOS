...
class SignUpViewModelTest {
    ...
    @Test
    fun `이메일 미입력 상태에서 다음 단계 시 에러 메시지`() = runTest {
        val viewModel = SignUpViewModel()
    ...
    @Test
    fun `이메일 입력 후 onNextStep 호출 시 단계 이동`() = runTest {
        val viewModel = SignUpViewModel()
    ...
    @Test
    fun `모든 입력 후 최종 단계에서 메인으로 이동`() = runTest {
        val viewModel = SignUpViewModel()
